...
class MyGUI:
	...
	def add(self):
		...
		x += pdf.w + dx
		exit = Button(self.mw, (x,y), 'exit', self.exit)
		self.mw.add(exit)
	...
	def sel_eraser(self):
		self.erase_stroke = not self.erase_stroke
		self.tool = 'eraser'
	...
	def show_eraser(self):
		...
		if self.erase_stroke or self.tool=='pen':
			c = (200,200,200)
		else:
			c = (200,200,0)
		pygame.draw.rect(sc, c, rect)
	...
	def remove(self, pos):
		if self.erase_stroke:
			self.remove_stroke(pos)
		else:
			self.remove_points(pos)

	def remove_points(self, pos):
		r = 1
		new_note = []
		for stroke in self.note[self.p]:
			...
			if drop: 
				...
				new_note.append(stroke)
		self.note[self.p] = new_note
		self.mw.update_display()

	def remove_stroke(self, pos):
		r = 3
		new_note = []
		for stroke in self.note[self.p]:
			s, c, pts = stroke
